...
- **`products_view.dart` & `solar_plant_form_card.dart`:** Gestão completa de catálogo com ramificação e categorização:
  ...
  2. **Formulário Especial de Conjunto de Produtos (`SolarPlantFormCard`):**
     - **Importação Inteligente com IA Google Gemini Vision (`GeminiSolarVisionService` & `SolarPdfImportDialog`):**
       - **Visão Computacional & OCR Multimodal:** Integração direta com a API do **Google Gemini (Gemini 1.5 Flash Vision)** para analisar qualquer cotação em PDF, Imagem (JPG, PNG, WEBP) ou texto colado.
       ...
       - **Gerenciador de Chave API:** Permite salvar a chave da API do Google AI Studio localmente no app (`SharedPreferences`) para nunca mais precisar redigitar.
