...
- **Gestão e Mapeamento de Telhados Fotovoltaicos via Satélite & Foto de Drone:**
  ...
  - **Persistência Completa no Cloud Firestore (`roof_studies`) & Firebase Storage:**
    ...
    - **Anotações de Orientação, Bússola e Irradiação Integradas:** Persistência total do Norte magnético (`DroneNorthCompass`), setas de orientação de quedas d'água (`DroneRoofArrow` com `sectionId`, ângulo e rotação), cores e comprimentos customizados, além de parâmetros oficiais de CEP, Estado (UF), Região, HSP diário CRESESB e cálculo de geração estimada consolidado em kWh/mês com fator angular por água.
