...
class _RegisterPageState extends State<RegisterPage> {
  ...
  void initState() {
    super.initState();
    _nameCtrl.text = controller.name;
    _emailCtrl.text = controller.email;
    _passwordCtrl.text = controller.password;
    _confirmPasswordCtrl.text = controller.confirmPassword;
  }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ENVIO DE INFORMAÇÕES DE CADASTRO E DIALOG DE CONFIRMAÇÃO
  // ─────────────────────────────────────────────────────────────────────────
  Future<void> _handleRegisterSuccess(String registeredName, String registeredEmail) async {
    final currentUser = FirebaseAuth.instance.currentUser;

    // 1. Enviar e-mail de verificação oficial pelo Firebase Auth
    if (currentUser != null) {
      try {
    ...
    // 2. Registrar notificação de boas-vindas no Firestore (sem senha)
    if (currentUser != null) {
      try {
        final userRef = FirebaseFirestore.instance.collection('users').doc(currentUser.uid);
        await userRef.collection('notifications').add({
          'title': 'Bem-vindo ao TAOS CRM! 🚀',
          'body': 'Sua conta foi criada com sucesso com o e-mail $registeredEmail. Acesse a plataforma sempre com suas credenciais.',
          'type': 'welcome',
          'isRead': false,
          'createdAt': FieldValue.serverTimestamp(),
        });

        // Gravar também na coleção mail para gatilho de disparo de e-mail transacional
        await FirebaseFirestore.instance.collection('mail').add({
          'to': [registeredEmail],
          'message': {
            'subject': 'Bem-vindo ao TAOS CRM - Confirmação de Cadastro',
            'html': '''
              <div style="font-family: Arial, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #00b4d8;">Bem-vindo ao TAOS CRM, $registeredName!</h2>
                <p>Sua conta foi criada com sucesso no sistema.</p>
                <div style="background: #f8fafc; padding: 16px; border-radius: 8px; border-left: 4px solid #00e5a3;">
                  <p style="margin: 4px 0;"><strong>Nome Completo:</strong> $registeredName</p>
                  <p style="margin: 4px 0;"><strong>E-mail de Acesso:</strong> $registeredEmail</p>
                  <p style="margin: 4px 0;"><strong>Data de Cadastro:</strong> ${DateTime.now().day.toString().padLeft(2, '0')}/${DateTime.now().month.toString().padLeft(2, '0')}/${DateTime.now().year}</p>
                </div>
                <p style="font-size: 13px; color: #64748b; margin-top: 16px;">
                  🔒 <em>Por segurança, sua senha nunca é compartilhada por e-mail ou armazenada em texto puro. Caso a esqueça, utilize o botão "Esqueci minha senha" na tela de login.</em>
                </p>
              </div>
            ''',
          },
    ...
    // 3. Resetar preferências locais de onboarding para novo ciclo
    try {
    ...
    // 4. Exibir Diálogo Informativo com as credenciais (sem senha) antes de prosseguir
    if (mounted) {
      await showDialog(
        context: context,
        ...
        builder: (ctx) => AlertDialog(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: const LinearGradient(
                    colors: [Color(0xFF00B4D8), Color(0xFF00E5A3)],
                  ),
          ...
          content: Column(
            ...
            children: [
              Text(
                'Enviamos as orientações de boas-vindas para seu e-mail. Guarde seus dados de acesso:',
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        const Icon(Icons.person_outline_rounded, size: 16, color: Color(0xFF00B4D8)),
                    ...
                    Row(
                      children: [
                        const Icon(Icons.mail_outline_rounded, size: 16, color: Color(0xFF00B4D8)),
                        const SizedBox(width: 8),
                        Text(
                          'Login / E-mail:',
                          style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: const Color(0xFF64748B)),
                    ...
                    Row(
                      children: [
                        const Icon(Icons.lock_outline_rounded, size: 16, color: Color(0xFF10B981)),
                        const SizedBox(width: 8),
                        Text(
                          'Senha:',
                          style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: const Color(0xFF64748B)),
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Por motivos estritos de segurança, senhas nunca são enviadas por mensagem ou e-mail.',
  ...
  Future<void> _handleQuickRegister() async {
    ...
    try {
      ...
      if (mounted) {
        await _handleRegisterSuccess(name, email);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned.fill(
            child: CustomPaint(
              painter: _RegisterCyberGridPainter(),
            ),
          ),
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            // ── COLUNA 2: CENA 3D (MONITOR SAINDO DE DENTRO DO CARD DE REGISTRO) ──
            Expanded(
              flex: 64,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // CENA HOLOGRÁFICA: MONITOR 3D EMERGINDO DO CARD DE REGISTRO + BADGES FLUTUANTES
  // ─────────────────────────────────────────────────────────────────────────
  Widget _buildHolographicScene() {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          // 1. Feixes de Luz e Portal Holográfico emanando do Card de Registro
          Positioned.fill(
            child: CustomPaint(
              painter: _RegisterHolographicPortalPainter(),
            ),
          ),
            ...
            child: Row(
              ...
              children: [
                ...
                CustomPaint(
                  ...
                  painter: _RegisterCurvedArrowPainter(),
                ),
              ],
            ),
          ),

          // 3. O Monitor 3D Transparente Projetando-se para fora do Card de Registro
          Positioned(
            top: 50,
            bottom: 50,
            left: 0,
            right: 220, // Passa por trás do card de registro (largura 440), criando a sobreposição 3D
            child: Image.asset(
          ...
          // 8. O Card Branco de Registro (Fica na frente, de onde os hologramas emergem)
          Positioned(
            right: 0,
            width: 440,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // CARD BRANCO DE REGISTRO (LADO DIREITO)
  // ─────────────────────────────────────────────────────────────────────────
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Observer(
            builder: (_) => TextFormField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: GoogleFonts.inter(color: const Color(0xFF94A3B8), fontSize: 12.5),
                prefixIcon: const Icon(Icons.lock_reset_rounded, color: Color(0xFF64748B), size: 18),
                suffixIcon: IconButton(
                  icon: Icon(
          ...
          // Botão CRIAR CONTA GRÁTIS com Gradiente Ciano -> Turquesa Esmeralda
          Observer(
            builder: (_) => Container(
              height: 48,
              decoration: BoxDecoration(
              ...
              child: ElevatedButton(
                ...
                onPressed: (controller.isLoading || _isQuickLoading)
                    ? null
                    : () async {
          ...
          Container(
            ...
            child: OutlinedButton(
              ...
              onPressed: (controller.isLoading || _isQuickLoading)
                  ? null
                  : () async {
                      final success = await loginStore.loginWithGoogle();
                      if (success && mounted) {
                        Modular.to.navigate('/dashboard/');
                      }
                    },
          ...
          // Rodapé do Card: "Já tem uma conta? Acessar Login"
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text(
  ...
  Widget _buildGoogleIcon() {
    return SizedBox(
      ...
      height: 20,
      child: CustomPaint(
        painter: _RegisterGoogleLogoPainter(),
      ),
    );
  }
  ...
  Widget _buildDevFloatingWindow() {
    return Positioned(
      ...
      child: AnimatedSwitcher(
        ...
        child: _isDevToolsExpanded ? _buildExpandedDevWindow() : _buildMinimizedDevButton(),
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Botão 3: Ir para Login
            OutlinedButton.icon(
              onPressed: () => Modular.to.navigate('/auth/login'),
              icon: const Icon(Icons.login_rounded, size: 15, color: Color(0xFF38BDF8)),
              label: Text(
...
// ─────────────────────────────────────────────────────────────────────────
class _RegisterCyberGridPainter extends CustomPainter {
  @override
...
// ─────────────────────────────────────────────────────────────────────────
class _RegisterCurvedArrowPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
...
// ─────────────────────────────────────────────────────────────────────────
class _RegisterGoogleLogoPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
...
// ─────────────────────────────────────────────────────────────────────────
// PINTURA DO PORTAL HOLOGRÁFICO E FEIXES DE LUZ (SAINDO DO CARD DE REGISTRO)
// ─────────────────────────────────────────────────────────────────────────
class _RegisterHolographicPortalPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    // Ponto focal de onde os hologramas emergem: borda esquerda do card de registro
    final originX = size.width - 420;
